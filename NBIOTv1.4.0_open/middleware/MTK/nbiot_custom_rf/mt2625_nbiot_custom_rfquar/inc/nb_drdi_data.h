...
typedef enum
{
  ...
  RF_HW_IDX_SINGLE_BAND_PA = RF_HW_IDX_START,
  RF_HW_IDX_MAX_VAR_COUNT,
#else
  RF_HW_IDX_VAR1 = RF_HW_IDX_START,
  RF_HW_IDX_VAR2 = 1,
  RF_HW_IDX_VAR3 = 2,
  RF_HW_IDX_VAR4 = 3,
  RF_HW_IDX_MAX_VAR_COUNT,
...
typedef struct
{
  ...
  /* Access to default HW variant database, when DRDI is disabled.*/
  const NB_RFFE_CUSTOM_DATABASE_T *rf_custom_db_default_hw;
...
typedef struct
{
  ...
  /* Access to default HW variant database, when DRDI is disabled.*/
  const struct N1RfDefaultCalDatabase  *rf_calib_db_default_hw;
  /* No of available HW variants/DRDI sets in this customization */
  uint8_t                       no_of_available_hw_variants;
  /* Access to database pointers */

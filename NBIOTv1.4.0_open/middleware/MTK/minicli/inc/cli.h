/* Copyright Statement:
 *
 * (C) 2015-2017  MediaTek Inc. All rights reserved.
 *
 ...
 * are needed: cli_init() and cli_task(), as shown in the example below.
 *
 * - Step 1. Apply the configuration settings with the function cli_init().
 * - Step 2. Call cli_task() to read the user input characters and then call the corresponding user-defined handler function.
 * - Step 3. The function cli_task() repeats until the user logs out, which may or may not be implemented by the developer.
 * - Step 4. The control is returned to the caller if log out is requested by the user. This step may never be reached if log out is not implemented.
 *
 *    @code
 ...
 * or <b>mac</b>.
 *
 * If "ver" is supplied as from input source (by calling __io_getchar()),
 * the function <i>cli_ver()</i> will be called to handle this command. The
 * version information will be printed to the output using __io_putchar().
 *
 ...
 *
 * In MiniCLI, the parameter <b>len</b> indicates the number of tokens
 * supplied to the handler. The parameters supplied by the user on the CLI are
 * stored in the <b>param</b> array based on the order of input. The array
 * index starts from 0.
 *
 *    @code
 *
 ...
 *    @endcode
 *
 * In \link cli_t \endlink, a member <b>state</b> specifies the initial
 * authentication status. If <b>state</b> is set to 1, the CLI engine
 * skips the login function. If it is set to 0, the CLI engine calls the
 * function pointer #knock() with the <b>user ID</b> and <b>password</b>
 * for an authenticated access.
 *
 * In \link cli_t \endlink, the member <b>echo</b> controls what to display.
 * In this example <b>echo</b> is used when typing a password, to hide or show
 * it.
 *
 * Default value is 0.
 *
 ...
 *
 * @note    The return value of the handler function is 0. MiniCLI may be
 *          enhanced in the future to support the processing of different
 *          return values.
 */
...
struct cli_history_s {
    ...
    char            *parse_token;  /**< In MiniCLI engine, a command will be
                                     *  parsed into tokens that are similar to
                                     *  standard <i>main</i> function parameters
                                     *  <i>argc</i> and <i>argv</i>. An array of
                                     *  pointers for keeping <i>argv</i> shall be
                                     *  prepared and placed at here.
                                     */
 ...
 *              The login ID and password were hardcoded in source code as
 *              'admin' and 'password'.
 *
 * @param[in]   id          The entered <i>id</i> is compared with the string
 *                          'admin' to validate the login request.
 *
 * @param[in]   password    The entered <i>password</i> is compared with the
 *                          string 'password' to validate the login request.
 *
 * @retval      1 if id and password both matches hardcoded strings.
 *              0 if id or password does not match the hardcoded strings.
 *
 * @note        For demo purposes, this function can be used as a reference
 *              implementation for user-defined login checker. However, for
 *              mass production products, define a new function with the same
 *              parameter signature and supply it to the #cli_t.knock pointer in
 *              #cli_t and customize the checking with stored credentials,
 *              e.g. id/password pairs stored in NVRAM.
 */
uint8_t cli_login_example(char *id, char *password);

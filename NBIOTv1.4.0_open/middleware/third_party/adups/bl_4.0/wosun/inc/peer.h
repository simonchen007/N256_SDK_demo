...
#define PEER_FLASH_BLOCKSIZE    4096 // 8192
//#define PEER_FLASH_BLOCKSIZE    65536 
...
int updateFile(int platform_id, int tools_version, int type, char* oldfile, int oldfile_type, char* newfile, int newfile_type, char* patchfile, int patchfile_type, int stream_type);
...
void peer_logout(char *info, ...);
void peer_sprintf(char *buf, const char *fmt, ...);
...
int peer_fread(void * buf, int size, int times, ML_PEER_FILE fp);
int peer_fwrite(const void *  buf, int size, int times, ML_PEER_FILE fp);
...
int peer_get_flashblocksize();
int peer_get_flashblocknum(int length);
int peer_get_flashblockpos(int block_num);
int peer_get_totalflashsize();

ML_BOOL peer_isReadbackCheck();

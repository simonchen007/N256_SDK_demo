...
#define NB_NETIF_MAX_CLIENT 12
static nb_netif_network_status_callback_t g_nb_netif_client[NB_NETIF_MAX_CLIENT] = {NULL};
...
static int nb_cid_add(u8_t context_id, u32_t channel_id, u32_t protocal_type);
...
int nb_netif_bearer_info_ind(nbiot_bearer_info_struct *netinfo)
{
    ...
    NBNETIF_LOGI("cnmgr notify tcpip bearer is activated: %d, channel id: %d, cid %d, IP:%s\n",
                netinfo->is_activated, (int)netinfo->channel_id, netinfo->cid, netinfo->ip_addr);

    if(netinfo->is_activated == 0) {
        nb_net_info_del_t *nbnetif;
        u8_t cid = netinfo->cid;
        u32_t channel_id = netinfo->channel_id;
        NBNETIF_LOGI("bearer deactived\r\n");
...
static int nb_netif_add_netinfo(nb_net_info_add_t *netinfo)
{
    ...
    netif->modem_status = NB_NETWORK_AVAILABLE;
...
static void nb_netif_update_netinfo(u32_t channel_id, nb_netif_status_t status)
{
    struct netif *netif;
    u32_t context_id = 0;
    ...
    while(p != NULL) {
        if(p->channel_id == channel_id) {
            ...
            netif = netif_find_by_context_id(context_id);
            if (netif != NULL) {
                netif->modem_status = status;
            }
            p->modem_status = status;
        }
        p = p->next;
    }
}
...
static int nb_cid_add(u8_t context_id, u32_t channel_id, u32_t protocal_type)
{
    ...
    p->cid = context_id;
    p->modem_status = NB_NETWORK_AVAILABLE;
    p->type = protocal_type;
    p->next = NULL;
    if (q == NULL) {
...
static err_t nb_low_level_output(struct netif *netif, struct pbuf *p)
{
    ...
    g_netif_context->send_count ++;
    ret = mux_ap_send_data(channel_id, p->payload, p->len, (void *)p);
    NBNETIF_LOGI("sent=%p,channel=%d,ret=%d", p, (int)channel_id, ret);
...
static void nb_low_level_mux_callback(mux_ap_event_t event_id, void *data)
{
    ...
    switch (event_id) {
        ...
        case MUX_AP_EVENT_RECEIVE_COMPLETED: {
            ...
            channel_id = info->channel_id;
            cid = nd_get_cid_by_channel(channel_id);
            NBNETIF_LOGI("recved=%p,channel=%d, cid=%d", p, channel_id, cid);
            if(cid != 0) {
        ...
        case MUX_AP_EVENT_FLOW_CONTROL: {
            mux_ap_event_flow_control_t* p_info = (mux_ap_event_flow_control_t*)data;
            mux_ap_event_flow_control_t* flow_control = pvPortMalloc(sizeof(mux_ap_event_flow_control_t));
            if (flow_control == NULL){
                NBNETIF_LOGE("NO Memory\r\n");
            }
            memcpy(flow_control, p_info, sizeof(mux_ap_event_flow_control_t));
            nb_netif_send_message(MSG_ID_MUX_FLOW_CONTROL,(void *)flow_control);
            break;
        }
        default: {
            break;
    ...
    for (;;) {
        if (xQueueReceive(g_netif_context->qid, &msg_queue_data, portMAX_DELAY) == pdPASS) {
            ...
            if (MSG_ID_BEARER_INFO_DEL == msg_queue_data.msg_id) {
                ...
                nb_cid_add(data->cid, data->channel_id, data->type);
            ...
            }else if(MSG_ID_MUX_FLOW_CONTROL == msg_queue_data.msg_id) {
                mux_ap_event_flow_control_t* flow_control = (mux_ap_event_flow_control_t*)(msg_queue_data.msg_data);
                uint32_t index = 0;
                nb_netif_status_t status;
                if (flow_control->on_off == true){
                    status = NB_NETWORK_AVAILABLE;
                }else{
                    status = NB_NETWORK_BUSY;
                }
                nb_netif_update_netinfo(flow_control->channel_id, status);
                for(index = 0; index < NB_NETIF_MAX_CLIENT; index ++){
                    if (g_nb_netif_client[index] != NULL){
                        NBNETIF_LOGI("Broadcast network status: %d", status);
                        g_nb_netif_client[index](status);
                    }
                }
                vPortFree(flow_control);
            }
        }
    }
}
...
bool nb_netif_register_network_status_indication(nb_netif_network_status_callback_t call_back)
{
    ...
    for(index = 0; index < NB_NETIF_MAX_CLIENT; index ++){
        if (g_nb_netif_client[index] == NULL){
            g_nb_netif_client[index] = call_back;
            return true;
        }
    }
    return false;

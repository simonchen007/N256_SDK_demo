                                ...
                                lwm2m_object_t * objectP)
{
    ...
    instanceID = targetP->instanceId;
    if(NULL != g_nb_app_context.lwm2m_info)
    {
      serverP = g_nb_app_context.lwm2m_info->serverList;
      while(NULL != serverP)
      {
          if(serverP->secObjInstID == instanceID)
          {
              break;
          }else {
      ...
      if(NULL == serverP)
      {
          //LOG_I(lwm2m_c, "no server found in client");
      }
    }
    
    if ( (NULL == targetP) || (NULL == serverP) )
    {
        return COAP_404_NOT_FOUND;
    }
    ...
    do
    {
        ...
        case LWM2M_SERVER_LIFETIME_ID:             
            ...
            if(COAP_204_CHANGED == result)
            {
                serverP->lifetime = targetP->lifetime;
            }
            break;

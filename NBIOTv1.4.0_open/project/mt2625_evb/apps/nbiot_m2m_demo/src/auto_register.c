...
void auto_register_tel_conn_mgr_ext_cb(unsigned int app_id,
...
static int simcardtype = 99;  // 0---CT  ,1 --- UNI,99---others
...
static void auto_register_parse_json_packet(char *packet, uint32_t packet_len)
{
    ...
    if (*pcode == '0') {
		...
		//tangfeng.modify ,for actual use saving power
		#ifdef LL_AUTO_REG_NOT_RETRY
        auto_reg_task_free();
		#else
...
static uint32_t auto_register_create_json_packet(char *register_content)
{
	...
	if(1 == simcardtype){
		cJSON_AddStringToObject(root, "REGVER", AUTO_REG_REGVER_VALUE_UNI);
	}else{
...
static int32_t auto_register_query_iccid_callback(ril_cmd_response_t *rsp)
{
    ...
    if (rsp->res_code == RIL_RESULT_CODE_OK && param != NULL) {
        ...
        if (status != NVDM_STATUS_OK || strncmp((const char *)saved_iccid, g_auto_reg_cntx.iccid, AUTO_REG_ICCID_LEN)) {
            ...
            AUTO_REG_LOGI("[auto_reg]ICCID is same, no need do auto-register flow!!\r\n");
...
#ifdef AUTO_REG_UT
{
    ...
    if (result == 0) {
		...
		if(1 == simcardtype){
			ret = httpclient_post(&auto_reg_client, AUTO_REG_POST_URL_UNI, &client_data);
		}else{
			ret = httpclient_post(&auto_reg_client, AUTO_REG_POST_URL_CT, &client_data);
		}
        ...
        if (ret != HTTPCLIENT_OK) { 
			...
			auto_register_prompt_message(3); //tangfeng.add
            auto_reg_start_retry_timer();
            return;
        }
...
static void auto_register_prompt_message(uint32_t result)
{
       ...
       cmd_result.result_code = APB_PROXY_RESULT_UNSOLICITED;
        switch (result) {
             case 3: { //tangfeng.add
				strcpy(data, "Self Register: fail,retrying...\r\n");
                break;
            }			
             case 2: {
			 	if(1 == simcardtype){
					strcpy(data, "UNI-Self Register: Have already registered!\r\n");
				}else{
					strcpy(data, "CT-Self Register: Have already registered!\r\n");
				}
                break;
            }
            case 1: {
				if(1 == simcardtype){
					strcpy(data, "UNI-Self Register: Success!\r\n");
				}else{
            ...
            case 0: {
				if(1 == simcardtype){
					strcpy(data, "UNI-Self Register: Fail!\r\n");
				}else{
...
static void auto_register_active_bearer(void)
{
	...
	else if (!strncmp(g_auto_reg_cntx.imsi, "46001", 5) || !strncmp(g_auto_reg_cntx.imsi, "46006", 5) || !strncmp(g_auto_reg_cntx.imsi, "46009", 5))
	{
				
		 ip_addr_t ns;

		 simcardtype = 1; //uni sim card
...
{  
    ...
    switch(event_id)
    {
        case RIL_URC_ID_CPIN: {  //10
            ...
            //AUTO_REG_LOGI("[auto_reg] RIL_URC_ID_CPIN, pin = %s, cpin_ready = %d\n",pin->code, cpin_ready);
                                           ...
                                           tel_conn_mgr_pdp_type_enum pdp_type)
{
    ...
    AUTO_REG_LOGI("[auto_reg]info_type: %d, result: %d",info_type, result);
    auto_reg_msg = pvPortMalloc(sizeof(auto_reg_message_t));
    auto_reg_msg->message_id = info_type;

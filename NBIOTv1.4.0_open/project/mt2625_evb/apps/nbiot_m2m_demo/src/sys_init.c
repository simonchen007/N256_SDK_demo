...
void system_default_sleep_lock(void)
{
    ...
    status = nvdm_read_data_item("sleep_manager", "sys_default_lock", (uint8_t *)&sys_default_lock, &len);
    if (status != NVDM_STATUS_OK) {
        printf("nvdm status = %d\n", status);
        len = sizeof(sys_default_lock);
        nvdm_write_data_item("sleep_manager", "sys_default_lock", NVDM_DATA_ITEM_TYPE_RAW_DATA, (uint8_t *)&sys_default_lock, len);
    }
...
void system_init(void)
{
    ...
    if (DEEPER_SLEEP == rtc_power_on_result_external()) {
    ...
    printf("system initialize done@%d.\r\n", (int)hal_dvfs_get_cpu_frequency());
}

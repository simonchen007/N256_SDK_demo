...
typedef enum {
    HAL_RTC_DEFAULT = 0,                                    /* Keep default attribute. */
    HAL_RTC_ENABLE  = 1,                                    /* Enable attribute. */
    HAL_RTC_DISABLE = 2                                     /* Disable attribute. */
} hal_rtc_ctrl_attr_t;
...
/** @brief Callback function definition to handle the RTC eint. \n
 *  @param[in] user_data is a pointer to the data assigned by #hal_rtc_set_eint_callback(). \n
 */
...
/**
 * @brief This function sets the RTC time zone.
 * @param[in] quarter_hour represents how many quarter hours with Greenwich time, and it ranges from -48~48.
 *                  quarter_hour/4 can represent time zone. For example, if quarter_hour is 32, then time zone is UTC_E8.
 * @return #HAL_RTC_STATUS_INVALID_PARAM, an invalid quarter hour parameter is given.
 *         #HAL_RTC_STATUS_OK, the operation completed successfully.
 */
hal_rtc_status_t hal_rtc_set_quarter_hour(signed char quarter_hour);

/**
 * @brief This function gets the RTC time zone.
 * @param[in] pTime_zone is a pointer to a quarter hour which ranges from -48~48.
 * @return #HAL_RTC_STATUS_INVALID_PARAM, an invalid pointer parameter is given.
 *         #HAL_RTC_STATUS_OK, the operation completed successfully.
 */
 ...
 *         #If the operation has completed successfully, the system will power off.
 */
...
/**
 * @brief This function forces the chip into rtc mode.
 * @param[in] eint_wakeup is used to control the wake up attribute of rtc eint.
 *                    If the wake up attribute doesn't need to be changed, eint_wakeup shuold be set as HAL_RTC_DEFAULT.
 *                    If the system should not be waked up by eint event after it has entered forced_shut_down_mode, eint_wakeup should be set as HAL_RTC_DISABLE.
 *                    If the system should be waked up by eint event after system has entered forced_shut_down_mode, eint_wakeup should be set as HAL_RTC_ENABLE.
 * @param[in] tick_wakeup is used to control the wake up attribute of rtc tick.
 * @param[in] alarm_wakeup is used to control the wake up attribute of rtc alarm.
 * @return #HAL_RTC_STATUS_ERROR, entering rtc mode has failed as a wake up event occurred.
 *           #If the operation has completed successfully, the system will power off.
 */
...
/**
 * @brief This function forces resetting chip.
 * @return #HAL_RTC_STATUS_ERROR, resetting chip has failed.
 *         #If the operation has completed successfully, the system will be reset.
 */

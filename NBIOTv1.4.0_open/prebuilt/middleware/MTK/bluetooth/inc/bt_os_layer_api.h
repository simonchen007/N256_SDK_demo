...
/**
 * @brief     Sleep management suspend callback function pointer.
 * @return    None.
 */
typedef void (*bt_os_layer_sleep_management_suspend_callback_t)(void *data, uint32_t mode);
 ...
 */
uint8_t bt_os_layer_sleep_manager_acquire_sleeplock(uint8_t handle_index, uint8_t level);
 ...
 * @note        Call #hal_sleep_manager_set_sleep_handle() to set up the sleep handle, before calling this function.
 */
uint8_t bt_os_layer_sleep_manager_release_sleeplock(uint8_t handle_index, uint8_t level);

/**
 * @brief       This function get the RTC power on result.
 * @param[in]   void.
 * @return      uint32_t.
 */
uint32_t bt_os_layer_rtc_power_on_result_external(void);

/**
 * @brief       This function register sleep management suspend callback.
 * @param[in]   void.
 * @return      None.
 */
void bt_os_layer_sleep_management_register_suspend_callback(uint8_t module, bt_os_layer_sleep_management_suspend_callback_t callback, void *data, uint32_t mode);
 ...
 */
int16_t bt_os_layer_nvdm_init(void);
 ...
 */
int16_t bt_os_layer_nvdm_write_data_item(const char *group_name,
 ...
 * @note      Call this API in tasks or function callbacks, but not in the interrupt handlers.
 */
int16_t bt_os_layer_nvdm_read_data_item(const char *group_name,
                                  const char *data_item_name,
                                  uint8_t type,
                                  uint8_t *buffer,
                                  uint32_t *size);
 ...
 * @note      Call this API in tasks or function callbacks, but not in the interrupt handlers.
 */
int16_t bt_os_layer_nvdm_modem_reset_data_item(const char *group_name,
                                  const char *data_item_name);

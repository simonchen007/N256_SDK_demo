...
if [ "${argv[3]}" == "bl" ]; then
	...
	#build customer signal db
    TARGET_PATH=$CM4_TARGET_PATH_BAK	
	echo "build customer signal DB...."
	echo "make -C $TARGET_PATH BUILD_DIR=$OUT/obj OUTPATH=$OUT silox_ext_db_gen"
	make -C $TARGET_PATH BUILD_DIR=$OUT/obj OUTPATH=$OUT silox_ext_db_gen 2>> $OUT/err.log
